...
export default function ControlsPanel() {
  ...
  const [jobs, setJobs] = useState<string>(String(config.n_jobs))
  const [cap1, setCap1] = useState<string>(String(config.buffer_caps[0] ?? 5))
  const [cap2, setCap2] = useState<string>(String(config.buffer_caps[1] ?? 5))

  const onReset = useCallback(() => {
    setConfig({
      n_jobs: Number(jobs) || 100,
      buffer_caps: [Number(cap1) || 5, Number(cap2) || 5],
    })
    void reset()
